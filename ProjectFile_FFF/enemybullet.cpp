...
static	char *g_TextureEnemyBulletNomalName = (char*)"data\\TEXTURE\\bullet_normal.png";//テクスチャ名

int g_EnemyBulletLongTex;
static	ID3D11ShaderResourceView* g_TextureEnemyBulletLong;
static	char *g_TextureEnemyBulletLongName = (char*)"data\\TEXTURE\\bullet_long.png";//テクスチャ名

int g_EnemyBulletHpTex;
static	ID3D11ShaderResourceView* g_TextureEnemyBulletHp;
static	char *g_TextureEnemyBulletHpName = (char*)"data\\TEXTURE\\bullet_hp.png";//テクスチャ名
...
static	char *g_TextureEnemyBulletSlashName = (char*)"data\\TEXTURE\\bullet_hp.png";//テクスチャ名
...
void UpdateEnemyBullet()
{
	...
	for (int i = 0; i < ENEMYBULLET_NOMAL_MAX; i++)
	{
		if (g_EnemyBulletNomal[i].use == true)
		{
			...
			if (CollisionBB(g_EnemyBulletNomal[i].pos, pPlayer->pos - D3DXVECTOR2(0.0f,pPlayer->size.y / 2.5f), D3DXVECTOR2(g_EnemyBulletNomal[i].w, g_EnemyBulletNomal[i].h), D3DXVECTOR2(pPlayer->size.x,1.0f)))
			{
				g_EnemyBulletNomal[i].use = false;
				//スペシャルの被ダメ減少
	...
	BulletCol[0] -= BulletColAdd[0];
	if ((BulletCol[0] <= 0.0f)|| (BulletCol[0] >= 1.0f))
	{
		BulletColAdd[0] *= -1.0f;
	}
...
void DrawEnemyBullet()
{
	...
	switch (pConfig->cBulletCol)
	{
	case 0:
		//赤
		BulletCol[1] = 1.0f;
		BulletCol[2] = BulletCol[0];
		BulletCol[3] = BulletCol[0];
		break;
	case 1:
		//青
	...
	for (int x = 0; x < ENEMYBULLET_NOMAL_MAX; x++)
	{
		if (g_EnemyBulletNomal[x].use)
		{
			DrawSpriteColor(g_EnemyBulletNomalTex, g_EnemyBulletNomal[x].pos.x, g_EnemyBulletNomal[x].pos.y, g_EnemyBulletNomal[x].w, g_EnemyBulletNomal[x].h,
				0.0f, 0.0f, 1.0f, 1.0f, D3DXCOLOR(BulletCol[1], BulletCol[2], BulletCol[3], 1.0f));
		}
	}
	...
	for (int w = 0; w < ENEMYBULLET_TRANSLUCENT_MAX; w++)
	{
		if (g_EnemyBulletTrans[w].use)
		{
			DrawSpriteColor(g_EnemyBulletTransTex, g_EnemyBulletTrans[w].pos.x, g_EnemyBulletTrans[w].pos.y, g_EnemyBulletTrans[w].w, g_EnemyBulletTrans[w].h,
				0.0f, 0.0f, 1.0f, 1.0f, D3DXCOLOR(BulletCol[1], BulletCol[2], BulletCol[3], EnemyAlfa));
		}
	}
	for (int z = 0; z < ENEMYBULLET_HP_MAX; z++)
	{
		if (g_EnemyBulletHp[z].use)
		{
			...
			DrawSpriteColor(g_EnemyBulletHpTex, g_EnemyBulletHp[z].pos.x, g_EnemyBulletHp[z].pos.y, g_EnemyBulletHp[z].w, g_EnemyBulletHp[z].h,
				0.0f, (1.0f/3.0f)*(g_EnemyBulletHp[z].hp-1), 1.0f, 1.0f / 3.0f, D3DXCOLOR(BulletCol[1], BulletCol[2], BulletCol[3], 1.0f));
		}
	}
}
	...
	if (nowY < MAP_SIZE_Y)
	{
		for (int j = 0; j < MAP_SIZE_X; j++)
		{
			...
			case 1:
				...
				{
				case 0:
					atk = (int)BULLETLANE_1;
					break;
				case 1:
					atk = (int)BULLETLANE_2;
					break;
				case 2:
					atk = (int)BULLETLANE_3;
					break;
				case 3:
					atk = (int)BULLETLANE_4;
					break;
				case 4:
					atk = (int)BULLETLANE_5;
					break;
				default:
					break;
				}
				if (IsAtk)
				{
					for (int i = 0; i < ENEMYBULLET_NOMAL_MAX; i++)
					{
						...
						{
							g_EnemyBulletNomal[i].pos.x = (float)atk;
			...
			case 2:
				...
				if (IsAtk)
				{
					for (int k = 0; k < ENEMYBULLET_LONG_MAX; k++)
					{
						...
						{
							g_EnemyBulletLong[k].pos.x = (float)atk;
			...
			case 3:
				...
				if (IsAtk)
				{
					for (int i = 0; i < ENEMYBULLET_TRANSLUCENT_MAX; i++)
					{
						...
						{
							g_EnemyBulletTrans[i].pos.x = (float)atk;
			...
			case 4:
				...
				if (IsAtk)
				{
					for (int k = 0; k < ENEMYBULLET_HP_MAX; k++)
					{
						...
						{
							g_EnemyBulletHp[k].pos.x = (float)atk;
