	...
	float PaternNo, float uv_w, float uv_h, int NumPatern)
{
	...
	//頂点 V0をセット
	vertex[0].Position = D3DXVECTOR3((float)(-Size_W / 2), (float)(-Size_H / 2), 0.0f);
	vertex[0].Diffuse = col;
	vertex[0].TexCoord = D3DXVECTOR2(0.0f + offset, 0.0f);
	//頂点 V1をセット
	vertex[1].Position = D3DXVECTOR3((float)(+Size_W / 2), (float)(-Size_H / 2), 0.0f);
	vertex[1].Diffuse = col;
	vertex[1].TexCoord = D3DXVECTOR2(1.0f + offset, 0.0f);
	//頂点 V2をセット
	vertex[2].Position = D3DXVECTOR3((float)(-Size_W / 2), (float)(+Size_H / 2), 0.0f);
	vertex[2].Diffuse = col;
	vertex[2].TexCoord = D3DXVECTOR2(0.0f + offset, 1.0);
	//頂点 V3をセット
	vertex[3].Position = D3DXVECTOR3((float)(+Size_W / 2), (float)(+Size_H / 2), 0.0f);
	...
	//UV座標の計算
	int		no	= (int)PaternNo;	//パターン番号を整数値にする 小数点以下を捨てる
