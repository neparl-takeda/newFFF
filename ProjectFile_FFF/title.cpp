...
UIText* g_UITextStart{};
UIText* g_UITextVersion{};
...
HRESULT InitTitle(void)
{
	...
	g_TextureVerNumber = LoadTexture((char*)"data/TEXTURE/number_03.png");
	...
	//UiText START
	g_UITextStart = new UIText;
	g_UITextStart->Init();
	D3DXVECTOR2 uiTextPos   = { CENTER_X - 100.0f, LOGO_POS_Y + 550.0f};
	D3DXVECTOR2 uiTextSize  = { 100.0f ,100.0f };
	D3DXCOLOR   uiTextColor = { 1.0f,1.0f,1.0f,1.0f };
	g_UITextStart->SetUITextPram(uiTextPos, uiTextSize, uiTextColor, (char*)"START", FONT_GRADATION);

	//UiText Version
	g_UITextVersion = new UIText;
	g_UITextVersion->Init();
	uiTextPos	= { 150.0f, 1040.0f, };
	uiTextSize	= { 50.0f ,50.0f };
...
void UninitTitle(void)
{
	...
	g_UITextStart = nullptr;
	delete g_UITextStart;
}
...
void UpdateTitle(void)
{
	...
	SetVolume(g_BGMNo,		pConfig->cSoundBGMv);
	SetVolume(g_SE,			pConfig->cSoundSEv);
	SetVolume(g_SE_S,		pConfig->cSoundSEv);
	SetVolume(g_SE_START,	pConfig->cSoundSEv);
	SetVolume(g_VFFF,		pConfig->cSoundSEv);

	if (TitleFrame == 1)
	{	PlaySound(g_VFFF, 0);	}	//再生-フォルティシッシャモ

	if (TitleFrame == 120)
	{	PlaySound(g_BGMNo, 0);	}	//再生-BGM

	//スペースーキーが押されたらSCENE_STAGESELECTへ移行する
	if ((Keyboard_IsKeyDown(KK_SPACE))&&(pConfig->cFrame>=60))
	{
	...
	if (kakusicommand[9] == true && (IsButtonTriggered(0, XINPUT_GAMEPAD_START) || Keyboard_IsKeyDown(KK_ENTER)))
	{
	...
	//uiText START 動作
	D3DXVECTOR2 uiTextSize = { 100.0f + (TitleAlpha * 8.0f),100.0f + (TitleAlpha * 8.0f) };
	g_UITextStart->SetSize(uiTextSize);
}
